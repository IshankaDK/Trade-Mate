...
// Endpoint to get User's Average Holding Period
export const getUserAverageHoldingPeriod = async (
  req: Request,
  res: Response<StandardResponse<any>>
) => {
  try {
    ...
    return res.status(200).json({
      success: true,
      message: "Average holding period retrieved successfully.",
      data: averageHoldingPeriod,
    });
  } catch (error) {
    console.error("Error fetching average holding period:", error);
    return res.status(500).json({
      success: false,
      message: "Internal server error.",
      error: error instanceof Error ? error.message : String(error),
